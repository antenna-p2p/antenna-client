"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
exports.__esModule = true;
var PathEntry = /** @class */ (function () {
    function PathEntry() {
    }
    return PathEntry;
}());
var Privates = /** @class */ (function () {
    function Privates() {
    }
    return Privates;
}());
var privates = new WeakMap();
function create(data) {
    var fakeFunction = function () { };
    privates.set(fakeFunction, data);
    return new Proxy(fakeFunction, handler);
}
var handler = {
    get: function (target, prop) {
        var data = privates.get(target);
        return create({ root: data.root, path: data.path.concat([{ prop: prop }]) });
    },
    apply: function (target, thisArg, args) {
        var _a = privates.get(target), root = _a.root, path = _a.path;
        var prop = path[path.length - 1].prop;
        var isFunction = function (a) { return typeof a === 'function'; };
        var indexOfCallback = args.findIndex(isFunction);
        Object.assign(path[path.length - 1], { args: args, indexOfCallback: indexOfCallback });
        if (indexOfCallback < 0) {
            return create({ root: root, path: path.slice(0, -1).concat([{ args: args || [], prop: prop }]) });
        }
        (function () { return __awaiter(_this, void 0, void 0, function () {
            var handle, value;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, root];
                    case 1:
                        handle = _a.sent();
                        return [4 /*yield*/, handle.executionContext().evaluate(function (root, path) {
                                return new Promise(function (resolve, reject) {
                                    function execStep(object, entry) {
                                        if (entry.prop === 'then' && entry.indexOfCallback === 0) {
                                            resolve(object);
                                            return;
                                        }
                                        var value = object[entry.prop];
                                        if (entry.args && entry.indexOfCallback >= 0) {
                                            entry.args[entry.indexOfCallback] = resolve;
                                        }
                                        return entry.args ? value.apply(object, entry.args) : value;
                                    }
                                    function execPath(subroot, subpath) {
                                        return subpath.length ? execPath(execStep(subroot, subpath[0]), subpath.slice(1)) : subroot;
                                    }
                                    execPath(root, path);
                                });
                            }, handle, path)];
                    case 2:
                        value = _a.sent();
                        args[indexOfCallback](value);
                        return [2 /*return*/];
                }
            });
        }); })();
    }
};
function directJSHandle(handle) {
    return create({ root: handle instanceof Promise ? handle : Promise.resolve(handle), path: [] });
}
exports.directJSHandle = directJSHandle;
function getWindowHandle(page) {
    return directJSHandle(page.evaluateHandle('window'));
}
exports.getWindowHandle = getWindowHandle;
